...
- `personaKey`는 seed 관계, API 로그인, 로그에서 계정 유형과 상태를 식별하는 자연키다.
- `personaKey`는 `재미있는수식어-한글이름-계정상태` 형식을 따른다.
- 시나리오 정보는 `scenario key`와 아래 `핵심 용도`가 담당하며 `personaKey`에는 넣지 않는다.
- 현재 suffix는 `GENERAL_CONSUMER`의 `일반강습생`, `INSTRUCTOR_APPROVED`의 `승인강사`다.
- 추후 지원할 `SUSPENDED_CONSUMER`와 `INSTRUCTOR_PENDING`은 각각 `정지강습생`, `승인대기강사`를 사용한다.
- 한글 이름은 팀원이 담당하거나 소유한다는 뜻이 아니라, 익숙한 이름을 기억 장치로 차용한 것이다.

| nickname | personaKey | 핵심 용도 |
| --- | --- | --- |
| 대뜸 GOAT 성빈 | `대뜸GOAT-성빈-일반강습생` | 비발디 가격·결제 강습생 |
| 보법 다른 유정 | `보법다른-유정-승인강사` | 비발디 가격·결제 강사 |
| 폭룡적 예지 | `폭룡적-예지-일반강습생` | 하이원 초급 2인 |
| 느좋 그 자체 예림 | `느좋그자체-예림-일반강습생` | 휘닉스 보드 4인 |
| 감다살 유빈 | `감다살-유빈-일반강습생` | 곤지암 첫 스키 |
| 야르 선문 | `야르-선문-일반강습생` | 지산 중급 보드 |
| 난리자베스 채원 | `난리자베스-채원-일반강습생` | 알펜시아 후보 없음 |
| 도파민 풀충 나현 | `도파민풀충-나현-일반강습생` | 오크밸리 다중 요청 |
| 레전드 갱신 중인 지환 | `레전드갱신중인-지환-일반강습생` | 엘리시안 아동 5인 |
| 갑차기스러운 예슬 | `갑차기스러운-예슬-일반강습생` | 웰리힐리 복수 종목 |
| 기세로 다 해먹는 도연 | `기세로다해먹는-도연-승인강사` | 하이원 초급 2인 강사 |
| 폼 미친 성빈 | `폼미친-성빈-승인강사` | 하이원 5인 강사 |
| 뉴런 공유 중인 유정 | `뉴런공유중인-유정-승인강사` | 곤지암 중상급 강사 |
| 냅다 레전드 유빈 | `냅다레전드-유빈-일반강습생` | 활성 요청 없는 자유 QA 시작점 |

### Scenario seed

| scenario key | 종류 | 용도 | 주의점 |
| --- | --- | --- | --- |
| `matching-price-vivaldi` | FLOW / STABLE | 매칭 요청부터 85,000원 결제와 강습 확정까지 검증 | 로컬/CI 전용 기본 골든 플로우 |
| `matching-no-candidate-alpensia` | FLOW / STABLE | 후보가 없을 때 `SEARCHING` 유지 검증 | 로컬/CI 전용, 즉시 `FAILED`가 되지 않음 |
| `matching-multi-request-oak` | FLOW / STABLE | 한 소비자가 활성 요청을 취소하며 요청 4건과 참가자 16명의 이력을 순차 생성 | 로컬/CI 전용, 동시 활성 요청은 409로 거절 |
| `pm-full-requested-catalog` | SNAPSHOT / TRANSITION | PM 스프레드시트의 강사 노출과 요청 이력 자동 검증 | 로컬/CI 전용이며 공유 dev Reset에서는 선택 불가 |
...
main 자동 배포 직후와 수동 Reset 뒤에는 모두 base seed만 있는 idle 상태를 만든다.

1. `main` ref에서 workflow를 연다. Seed 대상은 고르지 않으며 항상 `idle-base`다.
2. 기존 dev 데이터를 지우는 `confirmReset`을 체크한다.
3. workflow가 앱 중지 → clean → migrate → base → verify → Dev QA 시작 상태 검사 → 앱 재기동을 실행한다.
4. 결과 요약이 성공인지 확인한 뒤 새로 로그인해 QA를 시작한다.

Dev QA 시작 상태 검사는 현재 강사 수를 하드코딩하지 않는다. 강사가 추가되더라도 모든 강사가
승인 완료 상태이고 활성 가격 정책을 정확히 하나씩 가지는지 확인한다. 동시에 매칭 요청·그룹·
제안·가격 snapshot·결제·강습 관련 테이블이 모두 0건이고, 매칭 받기가 켜진 강사도 0명인지
확인한다. 하나라도 어기면 앱을 다시
켜기 전에 reset을 실패시킨다. 자동 CD는 더 구체적인 `verify-base.sql`에서 현재 QA 계정과
강사 4명, 승인·가격, 매칭 0건을 이미 검사하므로 이 공통 검사를 중복 실행하지 않는다.
...
계속 사용하면 안 된다. 일반 QA는 `idle-base`로 시작한다. 요청을 미리 넣는
`matching-*`, `pm-full-requested-catalog` 시나리오는 자동 테스트와 로컬 확인에만 남겨 두고,
공유 dev Reset 선택지와 실행기에서는 모두 차단한다.
...
curl -X POST http://localhost:8080/dev/auth/token \
  ...
  -d '{"personaKey":"대뜸GOAT-성빈-일반강습생"}'

curl -X POST http://localhost:8080/dev/auth/token \
  -H 'Content-Type: application/json' \
  -d '{"personaKey":"보법다른-유정-승인강사"}'
```
...
서버를 실행한다. 공유 dev QA에서는 이 scenario 자체를 선택할 수 없다.
