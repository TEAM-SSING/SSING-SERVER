...
curl() {
  ...
  if [[ "$joined" == *"/dev/auth/personas"* ]]; then
    ...
    if [[ "${FAKE_FAIL_STAGE:-}" == "persona-encoding" ]]; then
      printf '{"data":{"personas":[{"personaKey":"냅다레전드-유빈-자유QA-강습생","nickname":"냅다 레전드 유빈"},{"personaKey":"대뜸GOAT-성빈-비발디가격결제-강습생","nickname":"ëë¸ GOAT ì±ë¹"}]}}\n'
    else
      printf '{"data":{"personas":[{"personaKey":"냅다레전드-유빈-자유QA-강습생","nickname":"냅다 레전드 유빈"},{"personaKey":"대뜸GOAT-성빈-비발디가격결제-강습생","nickname":"대뜸 GOAT 성빈"}]}}\n'
    fi
    return 0
  fi
  return 22
